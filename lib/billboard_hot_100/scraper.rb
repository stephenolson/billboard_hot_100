...
class BillboardHot100::Scraper
  ...
  def self.scrape_billboard
    ...
    index.css("div.chart-list-item").each do |song|
      ...
      song_info[:title] = song.css(".chart-list-item__title-text").text.strip
      ...
      song_info[:lyrics] = song.css('div.chart-list-item__lyrics a').map { |link| link['href'] }.join
      song_info[:award] = song.css(".chart-list-item__award-icon").text.strip
